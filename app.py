...
def generate_plots(N, mu, sigma2, S):
    ...
    for _ in range(S):
        ...
        Y_sim = np.random.normal(mu, sigma2, N)  # Replace with code to generate Y values

        # TODO: Fit a linear regression model to X_sim and Y_sim
        sim_model = LinearRegression().fit(X_sim, Y_sim)  # Initialize model
        None  # Replace with code to fit model

        slope = sim_model.coef_[0]
        intercept = sim_model.intercept_ 
        # TODO: Append the slope and intercept of the model to slopes and intercepts lists
        slopes.append(slope)  # Replace None with code to append slope
        intercepts.append(intercept)  # Replace None with code to append intercept
